...
/**
 * This example tells Message Bus to asynchronously create a blocklist report
 * <p/>
 ...
 */
public class SampleCreateBlocklistReport {

    private MessageBusReportingClient messageBusReportingClient;

    public SampleCreateBlocklistReport(MessageBusReportingClient messageBusReportingClient) {
        this.messageBusReportingClient = messageBusReportingClient;
    }

    public static void main(final String[] args) {

        SampleCreateBlocklistReport exampleGetReport = new SampleCreateBlocklistReport(MessageBusFactory
                .createReportingClient("<YOUR_API_KEY>", "https://api.messagebus.com"));

        exampleGetReport.examplePollAndSaveBlocklistToFile();

    }

    public void examplePollAndSaveBlocklistToFile() {
        final String fileName = "testBlocklist.csv";

        try {
            FileOutputStream fos = new FileOutputStream(fileName);

            final BlocklistReportRequest blocklistReportRequest = new BlocklistReportRequest(MessageBusReportingClient.ReportFormat.CSV);

            final ReportCreateResponse reportCreateResponse = messageBusReportingClient.createBlocklistReport(blocklistReportRequest);
